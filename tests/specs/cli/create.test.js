...
const BASE_DIR = path.join(repoRoot, "tests/fixtures/cli/create");
...
describe("create command", function () {
  ...
  afterEach(function () {
    ...
    if (fs.existsSync(BASE_DIR)) {
      ...
      fs.mkdirSync(BASE_DIR, { recursive: true });
      fs.rmSync(
        path.join(repoRoot, "packages/cli/src/templates/broken-template"),
        {
          recursive: true,
          force: true,
        },
  ...
  it("creates a new NW.js application using template", function () {
    ...
    assert.strictEqual(fs.existsSync(path.join(BASE_DIR, "demo")), true);
  });
