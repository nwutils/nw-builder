...
const fixturesDir = path.join(repoRoot, "tests/fixtures/getter");
const cacheDir = path.join(repoRoot, "packages/getter/cache");
...
describe("getter test suite", function () {
  ...
  it("parses manifestUrl file:/// path correctly", async function () {
    await get({
      ...
      arch: "x64",
      downloadUrl: "https://dl.nwjs.io",
      manifestUrl: `file:///${path.join(fixturesDir, "main_manifest.json")}`,
      cacheDir,
      cache: true,
      ffmpeg: false,
      nativeAddon: false,
  ...
  it("throws when manifestUrl file:/// path is not valid JSON", async function () {
    await assert.rejects(
      get({
        ...
        arch: "x64",
        downloadUrl: "https://dl.nwjs.io",
        manifestUrl: `file:///${path.join(fixturesDir, "request_test.txt")}`,
        cacheDir,
        cache: true,
        ffmpeg: false,
        nativeAddon: false,
