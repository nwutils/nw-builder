  ...
  this._forEachPlatform(function (name, platform) {
    self._files.forEach(function (file) {
      if (path.extname(file.src) === ".json" && file.src.indexOf("package") === -1 && file.src.indexOf("bower") === -1) {
        ...
        var iv = CryptoJS.enc.Utf8.parse('e84ad660c4721ae0e84ad660c4721ae0');
        var Salt = CryptoJS.enc.Utf8.parse("insight123resultxyz");
        var key128Bits1000Iterations = CryptoJS.PBKDF2(encryptionPhrase, Salt, { keySize: 128 / 32, iterations: 1000 });
        var contents = fs.readFileSync(file.src, "utf8");
        var data = Buffer.from(contents).toString('base64');
        var encryptedContents = CryptoJS.AES.encrypt(data, CryptoJS.enc.Utf8.parse(encryptionPhrase).concat(key128Bits1000Iterations),
          {
        ...
        */
        ...
        fs.writeFileSync(path.join(directory, name, fileName), encryptedContents);
...
NwBuilder.prototype.encryptImages = function () {
  ...
  self._files.forEach(function (dataFile) {
    ...
    var fileName = path.basename(dataFile.src);
    if (fileName === "icon.png") {
      ...
      fileName = path.basename(dataFile.src).replace(extension, self.options.encryptedImageExtension);
      ...
      var encryptedContents = CryptoJS.AES.encrypt(contents, CryptoJS.enc.Utf8.parse(encryptionPhrase).concat(key128Bits1000Iterations),
        {
          mode: CryptoJS.mode.CBC,
          iv: iv,
          padding: CryptoJS.pad.Pkcs7
