...
function NwBuilder(options) {
    ...
    if (this.options.platforms.length === 0)
        throw new Error('No platform to build, sorry!');
...
NwBuilder.prototype.platformFilesForVersion = function () {
    ...
    this._forEachPlatform(function (name, platform) {
        platform_downloads = platforms[name].hasOwnProperty('downloads') ?
            Object.assign(self._version.platforms, platforms[name].downloads) :
            self._version.platforms;
        var satisfied = self.preparePlatformFiles(name, platform);
        if (!(satisfied && !!platform_downloads[name + "-" + self._version.flavor])) {
    ...
    this._forEachPlatform(function (name, platform) {
        ...
        if(platform.files.length === 1 && platform.files[0] === '*'){
            ...
            totalFiles.forEach(function(file){
                ...
                copyDestFile = path.join(platform.releasePath, destFile)

                copiedFiles.push(Utils.copyFile(file, copyDestFile, self, options));
                platform.files.push(copyDestFile);
            });
...
NwBuilder.prototype.handleMacApp = function () {
    ...
    this._forEachPlatform(function (name, platform) {
        ...
        console.log("version", self._version.version)
        if (name === 'osxarm') {
            srcFile = path.join(platform.cache, 'libffmpeg.dylib');

            destFile = path.resolve(platform.releasePath, self.options.appName + '.app', 'Contents', 'Frameworks/nwjs\ Framework.framework/Versions/Current', 'libffmpeg.dylib');
            allDone.push(Utils.copyFile(srcFile, destFile, self));

            // also need it in cache so that can run
            destFile = path.resolve(platform.cache, 'nwjs.app', 'Contents', 'Frameworks/nwjs\ Framework.framework/Versions/Current', 'libffmpeg.dylib');
            allDone.push(Utils.copyFile(srcFile, destFile, self));
        }
...
NwBuilder.prototype.preparePlatformFiles = function(platformName, platform, version) {
    ...
    if (platforms[platformName] && Object.keys(platform.files)[0] !== Object.keys(platforms[platformName].files)[0]){
        return true;
    }
