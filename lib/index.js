  ...
  this._forEachPlatform(function (name, platform) {
    self._files.forEach(function (file) {
      if (path.extname(file.src) === ".js" && fs.readFileSync(file.src, 'utf8').length > 0) {
        ...
        const args = [
          resolvedPath,
          path.join(self.options.compiledDir, baseName).replace(".js", "") + self.options.compiledFileExtension
        ];
        ...
        file.src = path.join(self.options.compiledDir, baseName).replace(".js", "") + self.options.compiledFileExtension;
