  ...
  this._forEachPlatform(function (name, platform) {
    self._files.forEach(function (file) {
      if (path.extname(file.src) === ".json") {
        ...
        file.src = path.join(directory, fileName.replace(".json", self.options.compiledJsonFileExtension));
    ...
    if (!zipping) {
      ...
      self._files.forEach(function (file) {
        ...
        if (file.dest === 'package.json' && platform.platformSpecificManifest) {
          ...
          if ((path.extname(file.src) !== ".js" || self.options.compile === false) && path.extname(file.src).indexOf("save") === -1) {
