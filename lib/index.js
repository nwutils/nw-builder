...
var platformOverrides = require("./platformOverrides");
...
if (packageUpdate) {
  var message =
    ...
    " update available (" +
    chalk().dim(packageUpdate.current) +
    chalk().reset(" → ") +
    chalk().green(packageUpdate.latest) +
    ")" +
    "\nTo update, run one of the following commands:" +
    "\n- " +
    chalk().cyan("npm i -g " + pkg.name) +
    " (if " +
    pkg.name +
    " is installed globally)" +
    "\n- " +
    chalk().cyan("npm i -D " + pkg.name) +
    " (if " +
    pkg.name +
        ...
        if (["nw", "nwjs.app", "nw.exe"].indexOf(destFile) !== -1) {
          ...
          if (destFile === "nwjs.app") {
            options.filter = function(filepath) {
              return !/nwjs\.app\/Contents\/Resources\/[^.]+\.lproj(\/|$)/.test(
                filepath
              );
            };
          }
...
NwBuilder.prototype.runApp = function() {
  ...
  return new Promise(function(resolve, reject) {
    var parentDirectory = (_.isArray(self.options.files)
      ? self.options.files[0]
      : self.options.files
    ).replace(/\*[\/\*]*/, "");
    var nwProcess = (self._nwProcess = spawn(
      executable,
      ["--enable-logging", parentDirectory].concat(self.options.argv)
    ));
