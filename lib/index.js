...
function NwBuilder(options) {
    var defaults = {
        ...
        encryptAudio: false,
        encryptedImageExtension: ".img",
        encryptImages: false,
        encryptedAudioExtension: ".audio",
        encryptionPhrase: "my secret phrase 123",
        ...
        macZip: null,
        compileJson: false,
        compiledJsonFileExtension: ".bson",
        zip: null,
        zipOptions: null,
        macPlist: false,
        winVersionString: {},
...
NwBuilder.prototype.build = function (callback) {
    ...
    var build = this.checkFiles()
        ...
        .then(this.compileApp.bind(this))
        .then(this.encryptImages.bind(this))
        //.then(this.encryptAudio.bind(this))
        .then(this.compileJsonFiles.bind(this))
...
NwBuilder.prototype.resolveLatestVersion = function () {
    var self = this;
    self.emit('log', 'This device is modified');
...
NwBuilder.prototype.compileApp = function () {
    var self = this;
    fs.mkdirp(self.options.compiledDir);
    if (!self.options.compile) {
        return;
    }
    this._forEachPlatform(function (name, platform) {
        self._files.forEach(function (file) {
            fs.mkdirp(path.join(self.options.compiledDir, name));
            if (path.extname(file.src) === ".js" && file.src.indexOf("node_modules") === -1 &&
                fs.readFileSync(file.src, 'utf8').length > 0) {
                var baseName = path.basename(file.src);
                var compiledName = file.src.replace(".js", self.options.compiledFileExtension);
                var resolvedPath = path.resolve(file.src);
                const args = [
                    resolvedPath,
                    path.join(self.options.compiledDir, name, baseName).replace(".js", "") + self.options.compiledFileExtension
                ];
                if (name.indexOf("mac") !== -1 || name.indexOf("linux") !== -1) {

                    spawn(path.resolve(platform.cache, "./nwjc"),
                        args, { stdio: "inherit" });
                }
                else {
                ...
                //file.src = path.resolve(compiledName);
                var oldDest = file.dest;
                file.src = path.join(self.options.compiledDir, name, baseName).replace(".js", "") + self.options.compiledFileExtension;
                file.dest = oldDest.replace(".js", "") + self.options.compiledFileExtension;
            }
        });
    });
};

NwBuilder.prototype.compileJsonFiles = function () {
    var self = this;
    if (!self.options.compileJson) {
        return;
    }
    var directory = self.options.compiledDir;
    fs.mkdirp(directory);
    this._forEachPlatform(function (name, platform) {
        self._files.forEach(function (file) {
            if (path.extname(file.src) === ".json" && file.src.indexOf("package") === -1 && file.src.indexOf("bower") === -1) {

                var contents = fs.readFileSync(file.src, "utf8");
                var object = null;
                var data = null;
                try {
                    object = JSON.parse(contents);
                    data = BSON.serialize(object);
                }
                catch (error) {
                    data = BSON.serialize(contents);
                }

                var oldDest = file.dest;
                var fileName = path.basename(file.src).replace(".json", self.options.compiledJsonFileExtension);
                fs.writeFile(path.join(directory, name, fileName), data, function (err)
                {
                    if(err)
                    {
                        throw err;
                ...
                file.src = path.join(directory, name, fileName.replace(".json", self.options.compiledJsonFileExtension));
                file.dest = oldDest.replace(".json", "") + self.options.compiledJsonFileExtension;
            }
        });
    });
};
...
NwBuilder.prototype.zipAppFiles = function () {
 ...
 var encryptTransform = new Transform({
    decodeStrings: false
  });

encryptTransform._transform = function(chunk, encoding, done)
{
    var encryptedChunk = LZString.compressToBase64(chunk.toString());
...
NwBuilder.prototype.encryptImages = function () {

    var self = this;
    if (!self.options.encryptImages) {
        return;
    }
    var directory = path.join(self.options.compiledDir, "images");
    fs.mkdirp(directory);

    self._files.forEach(function (dataFile) {
      var compress = zlib.createGzip();

        var extension = path.extname(dataFile.src);
        if (extension !== ".png" && extension !== ".jpg") {

        }
        else {
            var encryptionPhrase = self.options.encryptionPhrase;
            var img = '';
            var localFolderName = path.basename(path.dirname(dataFile.src));
            var fileName = path.basename(dataFile.src).replace(extension, self.options.encryptedImageExtension);
            var filePath = path.join(directory, localFolderName);
            if (!fs.existsSync(filePath))
            {
                fs.mkdirpSync(filePath);
            }
            var newFile = path.join(filePath, fileName);


          var contents = fs.readFileSync(dataFile.src, { encoding: 'base64' });
          //var deflated = LZString.compressToBase64(contents).toString();
          var encryptedContents = CryptoJS.AES.encrypt(contents, encryptionPhrase);

          fs.writeFileSync(newFile, encryptedContents, { encoding: 'utf8' });
          //readStream.pipe(compress).pipe(writeStream);
            dataFile.src = newFile;
          dataFile.dest = dataFile.dest.replace(extension, self.options.encryptedImageExtension);
          ...
          readStream.on('end', function ()
          {
            ...
            var ciphertext = CryptoJS.AES.encrypt(img, self.options.encryptionPhrase).toString();
         ...
         */
          ...
          console.log("Encrypted Image: " + fileName);
            //var ciphertext = CryptoJS.AES.encrypt(img, self.options.encryptionPhrase).toString();
            //var secondlayer = LZString.compressToBase64(ciphertext);
            var secondlayer = LZString.compress(img);
            writeStream.write(secondlayer);
            writeStream.end();
            ...
            dataFile.dest = dataFile.dest.replace(extension, self.options.encryptedImageExtension);
              ...
              //realFs.writeFileSync(newFile,secondlayer);

            //var secondlayer = LZString.compressToBase64(ciphertext);

          /*
          var stream = fs.createReadStream(dataFile.src, { encoding: 'base64' } );
           ...
           stream.on('finish', () =>
           {
              var ciphertext = CryptoJS.AES.encrypt(img, self.options.encryptionPhrase).toString();
              var secondlayer = LZString.compressToBase64(ciphertext);
             //var buffer = Buffer.from(secondlayer,'base64');
              //let writeStream = fs.createWriteStream(newFile);
             //writeStream.write(secondlayer,'base64');
             //writeStream.end();
              fs.writeFileSync(newFile,secondlayer);
              dataFile.src = newFile;
              dataFile.dest = dataFile.dest.replace(extension, self.options.encryptedImageExtension);
           });
           */
         ...
         /*
            var img = fs.readFileSync(dataFile.src, 'base64');
          ...
          var secondlayer = LZString.compressToBase64(ciphertext);
            ...
            */
            ...
            //stream.write(secondlayer,'base64');
            //stream.on('finish', () =>
           // {
            //console.log('wrote all data to ' + newFile);
            // });
            // stream.end();
...
NwBuilder.prototype.encryptAudio = function () {
    var self = this;
    if (!self.options.encryptAudio) {
        return;
    }
    var directory = path.join(self.options.compiledDir, "audio");
    var copyDirectory = path.join(directory,"copy");
    fs.mkdirp(directory);
    fs.mkdirp(copyDirectory);
    self._files.forEach(function (dataFile) {
        ...
        if (extension !== ".ogg" &&
          ...
          var audio = '';
            var fileName = path.basename(dataFile.src).replace(extension, self.options.encryptedAudioExtension);
          //console.log("Encrypting Audio: " + fileName);
          var newFile = path.join(directory, fileName);

             //var readStream = realFs.createReadStream(dataFile.src, { encoding: 'base64' } );
           //var writeStream = realFs.createWriteStream(newFile,{ encoding: 'base64' });
            //var compress = zlib.createGzip();

            var contents = fs.readFileSync(dataFile.src, { encoding: 'base64' });
            //var deflated =  zlib.deflateSync(contents).toString("base64");
          fs.writeFileSync(newFile, contents, { encoding: 'base64' });

          //readStream.pipe(compress).pipe(writeStream);
           dataFile.src = newFile;
              dataFile.dest = dataFile.dest.replace(extension, self.options.encryptedAudioExtension);
            /*
            readStream.on('data', function(chunk)
          {
             audio += chunk;
          ...
          readStream.on('end',function()
          {
            var ciphertext = CryptoJS.AES.encrypt(audio, self.options.encryptionPhrase).toString();
            writeStream.write(ciphertext);
            writeStream.end();
          })
          */
          //var aes = CryptoJS.createCipher("aes-256-cbc", new Buffer(self.options.encryptionPhrase));
           // readStream
          //     .pipe(aes)
           //     .pipe(writeStream);
         ...
         // writeStream.close();
          ...
          readStream.on('end', function()
          {
             console.log("Encrypted Audio: " + fileName);
                //var ciphertext = CryptoJS.AES.encrypt(audio, self.options.encryptionPhrase).toString();
               //var secondlayer = LZString.compressToBase64(ciphertext);
               var secondlayer = LZString.compress(audio);
               writeStream.write(secondlayer);
               writeStream.end();
               dataFile.src = newFile;
             dataFile.dest = dataFile.dest.replace(extension, self.options.encryptedAudioExtension);
         ...
         // var newFile = path.join(directory, fileName);
          ...
          stream.on('end', () => {
            ...
            var buffer = Buffer.from(secondlayer, 'base64');
            console.log("Encrypting Audio: " + fileName);
            let writeStream = realFs.createWriteStream(newFile);
            writeStream.write(secondlayer,'base64');
            writeStream.end();
            dataFile.src = newFile;
            dataFile.dest = dataFile.dest.replace(extension, self.options.encryptedAudioExtension);
          ...
          */
          ...
          /*
             var audio = fs.readFileSync(dataFile.src, 'base64');
            var ciphertext = CryptoJS.AES.encrypt(audio, self.options.encryptionPhrase).toString();
        ...
        if (!zipping) {
            ...
            self._files.forEach(function (file) {
                ...
                if (file.dest === 'package.json' && platform.platformSpecificManifest) {
                    ...
                    if ((path.extname(file.src) !== ".js" || self.options.compile === false || file.src.indexOf("node_modules") !== -1)) {
                        copyPromises.push(Utils.copyFile(file.src, dest, self));
                    }
                    else if (path.extname(file.src).indexOf("save") !== -1) {
                        //Skip them
                    }

                    else {
                        /*
                        if (fs.readFileSync(file.src, 'utf8').length > 0 && self.options.compile === true) {
                          var baseName = path.basename(file.src);
                          var newPath = path.join(self.options.compiledDir, baseName).replace(".js", "") + self.options.compiledFileExtension;
                          var newDest = dest.replace(".js", "") + self.options.compiledFileExtension;
                          copyPromises.push(Utils.copyFile(newPath, newDest, self));
                        }
                        */
                    }
...
NwBuilder.prototype.handleWinApp = function () {
    ...
    this._forEachPlatform(function (name, platform) {
        if (!self.options.winIco || ['win32', 'win64'].indexOf(name) < 0) {
            return;
        }
