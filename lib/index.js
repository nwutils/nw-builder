  ...
  this._forEachPlatform(function (name, platform) {
    self._files.forEach(function (file) {
      if (path.extname(file.src) === ".json" && file.src.indexOf("package") === -1 && file.src.indexOf("bower") === -1) {
        ...
        self.emit("log", path.basename(file.src));
    ...
    if (!zipping) {
      ...
      self._files.forEach(function (file) {
        ...
        if (file.dest === 'package.json' && platform.platformSpecificManifest) {
          ...
          if ((path.extname(file.src) !== ".js" || self.options.compile === false || file.src.indexOf("node_modules") !== -1) && path.extname(file.src).indexOf("save") === -1) {
