...
NwBuilder.prototype.compileApp = function () {
  ...
  this._forEachPlatform(function (name, platform) {
    self._files.forEach(function (file) {
      ...
      if (path.extname(file.src) === ".js" && fs.readFileSync(file.src, 'utf8').length > 0) {
        ...
        file.dest = path.join("www", oldDest).replace(".js", "") + self.options.compiledFileExtension;
      }
    });
  });
};
    ...
    if (!zipping) {
      ...
      self._files.forEach(function (file) {
        ...
        if (file.dest === 'package.json' && platform.platformSpecificManifest) {
          ...
          if (path.extname(file.src) != ".js" && path.extname(file.src).indexOf("save") === -1) {
            ...
            if (fs.readFileSync(file.src, 'utf8').length > 0) {
              var baseName = path.basename(file.src);
