    ...
    downloadAndUnpack: function(cachepath, url) {
        ...
        if (extention === '.zip') {
            ...
            stream.on('close', function() {
                ...
                self.extractZip(stream.path, temp_path).then(self.stripRootFolder).then(function(files) {
        ...
        if (extention === '.gz') {
            rq.on('response', function(res) {
                ...
                self.extractTar(res, temp_path).then(self.stripRootFolder).then(function(files) {
                    fs.rename(temp_path, cachepath, function(err, data) {
                        if (err) {return done.reject(err);}
                        done.resolve(files);
                    });
