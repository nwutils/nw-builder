...
/**
 * Maps files to platform, then gets the manifest versions returning a filtered array of version objects,
 * containing version, flavor, supportedPlatforms, and downloadUrl.
 *
 * @param  {string}            downloadUrl    Path to download server (I think)
 * @param  {string}            [manifestUrl]  Optional URL, defaults to 'https://nwjs.io/versions.json'
...
module.exports = {
  ...
  /**
   * Determines whether to get legacy or latest versions, based on the desired version,
   * then retrieves information for the desired version.
   *
   * @param  {object}  params                 Contains the desired version and urls used to get the version
   * @param  {string}  params.desiredVersion  The desired NW.js version number
   * @param  {string}  params.downloadUrl     Path to download server (I think)
   * @param  {string}  params.manifestUrl     Path to package.json (I think)
   * @param  {string}  params.flavor          Desired flavor of the NW.js version ('sdk' or 'normal')
   * @return {Promise}                        Resolves to a {Version}
   */
  getVersion: function (params) {
    let method = getVersionsFromManifest;
    if (isLegacyVersion(params.desiredVersion)) {
      method = getLegacyVersions;
    }

    return method(params.downloadUrl, params.manifestUrl)
      .then(function (versions) {
        const versionIndex = versions.findIndex(function (version) {
          return version.version === params.desiredVersion;
        });

        if (versionIndex >= 0) {
          return Promise.resolve(versions[versionIndex]);
        }
        return Promise.reject('Version ' + params.desiredVersion + ' not found.');
      });
  },
  /**
