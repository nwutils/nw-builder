...
NwBuilder.prototype.downloadNwjs = async function () {
  ...
  for await (let osName of options.platforms) {
    let plat = osName.slice(0, osName.length - 2);
      let arch =
        osName.slice(osName.length - 2, osName.length) === "32"
          ? "ia32"
          : "x64";
    if (
      fs.existsSync(
        `${this.options.cacheDir}/${this.options.version}-${this.options.flavor}/${osName}/nwjs-${this.options.flavor}-${this.options.version}-${plat}-${arch}`,
      ) === false
    ) {
      await install(
        ...
        `nwjs-${this.options.flavor}-${this.options.version}-${plat}-${arch}`,
      );
    }
  }
};
