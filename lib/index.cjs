...
const { develop } = require("nw-develop");
...
function NwBuilder(options) {
  let parsedOptions = parse(options);
  let packageOptions = checkPkgOptions(parsedOptions.files);
  if (Object.entries(packageOptions).length === 0) {
    this.options = parsedOptions;
  } else {
    this.options = packageOptions;
  }
  let isValid = validate(this.options);
  if (isValid === false) {
    console.log("Invalid options.");
    process.exit(1);
  }
  this.options.platforms = platformsParser(this.options.platforms);
  let packageJsonExists = false;
  for (let file of this.options.files) {
    if (!packageJsonExists && path.basename(file) === "package.json") {
      this.packageJson = fs.readFileSync(`${file}`, "utf8");
  ...
  if (packageJsonExists) {
    this.options.appName = this.options.appName ?? this.packageJson.name;
    this.options.appVersion = this.options.appVersion ?? this.packageJson.version;
  }
}
...
NwBuilder.prototype.run = async function () {
  let options = this.options;
  let platform = getPlatform(process);
  let arch = getArch(process);

  if (platform === null || arch === null) {
    console.log("Unsupported platform/arch.");
  }

  if (
    fs.existsSync(
      `${options.cacheDir}/${options.version}/${options.flavor}/${platform}/${arch}/nw`,
    ) === false
  ) {
    await install(
      options.version,
      ...
      "https://nwjs.io",
      "https://dl.nwjs.io",
      `${options.cacheDir}/${options.version}/${options.flavor}/${platform}/${arch}`,
      "nw",
    );
  }
  await develop(
...
NwBuilder.prototype.downloadNwjs = async function () {
  ...
  for await (let osName of options.platforms) {
    ...
    let arch = osName.slice(osName.length - 2, osName.length) === "32" ? "ia32" : "x64";
    if (this.options.forceDownload === true || fs.existsSync(`${options.cacheDir}/${options.version}/${options.flavor}/${plat}/${arch}`) === false) {
      await install(options.version, options.flavor, plat, arch, "https://nwjs.io", "https://dl.nwjs.io", `${options.cacheDir}/${options.version}/${options.flavor}/${plat}/${arch}`, "nw",);
    }
  }
};
