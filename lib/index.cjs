...
function NwBuilder(options) {
  ...
  let isValid = validate(parsedOptions);
  if (isValid === false) {
...
NwBuilder.prototype.build = async function () {
  ...
  for await (let osName of options.platforms) {
    ...
    if (fs.existsSync(`${options.cacheDir}/${options.version}/${options.flavor}/${plat}/${arch}`) === false) {
