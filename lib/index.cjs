...
function NwBuilder(options) {
  ...
  let validatedOptions = validate(parsedOptions);
  let packageOptions = checkPkgOptions(validatedOptions.files);
  if (Object.entries(packageOptions).length === 0) {
    this.options = validatedOptions;
  } else {
    this.options = packageOptions;
  }
  this.options.platforms = platformsParser(this.options.platform);
