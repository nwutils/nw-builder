...
const { checkCache, Options, parseOptions, Platforms, detectCurrentPlatform, validateOptions } = require("../dist/index.cjs");
...
function NwBuilder(options) {
  ...
  this.options = parseOptions(validatedOptions, Options)
  ...
  if (typeof this.options.platforms != "undefined") {
    ...
    if (this.options.platforms && this.options.platforms.indexOf("linux") >= 0) {
      this.options.platforms.splice(
  ...
  this.options.platforms && this.options.platforms.forEach(function (platform) {
