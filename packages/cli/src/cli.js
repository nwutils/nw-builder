...
program.name("nw").description("Command line utility for NW.js applications");
...
program
  ...
  .action(
    (name /** @type {string} */, options /** @type {CreateOptions} */) => {
      create(name, options.template, options.outDir);
    },
  );

applyOptions(
  program.command("dev").description("Run app in development mode"),
