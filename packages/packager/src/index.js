 ...
 * land.
 * @typedef {"AppImage" | "deb" | "rpm" | "MSIX" | "NSIS"} PackageFormat
 */
 ...
 */
async function packageApp({ format, ...options }) {
  switch (format) {
    case "AppImage":
    ...
    default:
      throw new Error(
        `Expected options.format to be one of "AppImage", "deb", "rpm", "MSIX" or "NSIS". Got ${JSON.stringify(format)}.`,
      );
  }
}
